...
// Page de connexion 100% autonome (React + Tailwind)
...
//     return <LoginPage onLogin={async (email, password, remember) => {
//       // Remplacez par votre logique d'authentification
//       if(email === "demo@site.com" && password === "demo123") return { ok: true };
//       return { ok: false, message: "Identifiants invalides" };
//     }} />
//   }
//
// Props facultatives :
// - onLogin: (email, password, remember) => Promise<{ ok: boolean; message?: string }>
// - forgotPasswordUrl, signUpUrl: string
...
// Utilisation d'un gradient CSS au lieu d'une image externe pour éviter les problèmes CSP

export default function Login({ config }: Props): React.JSX.Element {
  const { onLogin, forgotPasswordUrl, signUpUrl, brand } = useLoginConfig(config);
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      // Convertir l'email en username pour l'AuthContext
      const username = email === "demo@site.com" ? "demo" : 
                      email === "admin@site.com" ? "admin" : 
                      email.split("@")[0];
      ...
      <div className="relative z-10  grid min-h-screen  grid-cols-1 items-center pl-4  sm:pl-6 lg:grid-cols-2 lg:gap-16 lg:pl-8">
        ...
        >
          ...
          <p className="mt-6 max-w-md text-balance text-lg text-white/70">
            Connectez-vous pour accéder à votre espace. Gérez vos projets, suivez vos
            statistiques et gagnez du temps au quotidien.
          </p>
            </div>
        ...
        <div 
          className="mx-auto w-full h-screen flex flex-col justify-center border-r border-white/10 bg-white/10 p-6 shadow-[0_10px_40px_rgba(0,0,0,.2)] backdrop-blur-xl sm:p-8"
        >
            ...
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
              ...
              <div className="flex items-center justify-between mb-10">
                ...
                <a href={forgotPasswordUrl} className="text-sm text-white/80 underline-offset-4 hover:underline">
                  Mot de passe oublié ?
                </a>
              </div>
              ...
              <p className="text-center text-sm text-white/70">
                Pas de compte ?{' '}
                <a href={signUpUrl} className="font-medium text-white underline-offset-4 hover:underline">
                  Inscrivez-vous
                </a>
              </p>
            </form>
