...
const Sidebar: React.FC<SidebarProps> = ({ className = "", onPluginClose }) => {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="px-3.5 py-2 mt-2.5 ">
      ...
      {/* Navigation Menu */}
      <nav className="mt-4 relative" style={{ width: sidebarVariants.expanded.width }}>
        {/* Indicateur animé */}
        
        <ul className="space-y-1 relative">
        <motion.div
          className={`absolute ${isExpanded ? 'rounded-r-box' : 'rounded-box'} bg-primary/10 z-0`}
          animate={{
            top: `${menuItems.findIndex((item) => item.isActive) * (37.59 + 4)}px`,
            left: isExpanded ? '0' : "5.5px",
            width: isExpanded ? '13.5rem' : "38px",
            height: "37.59px"
          }}
          transition={{
            type: "spring",
              ...
              <div className="relative group">
                ...
                >
                  ...
                  {!isExpanded && item.badge && (
                    <span className="absolute -top-1.5 -right-1.5 inline-flex items-center justify-center w-2 h-2 text-xs font-medium bg-red-500 text-white rounded-full">
                    </span>
                  )}
                  </div>
